...
label define oblast 2 "Issyk-kul" 3 "Jalalabat" 4 "Naryn" 5 "Batken" 6 "Osh" 7 "City of Osh" 8 "Chui" 11 "City of Bishkek"
label values oblast oblast
...
gen rural = .
replace rural = 0 if b002 == 1
replace rural = 1 if b002 == 2
label var rural "Rural"
label define rural 0 "Urban" 1 "Rural"
label values rural rural
gen logpccd = ln(pccd)
label var logpccd "Ln of per capita income, deflated"
gen lnhsize = ln(hsize)
gen hsize_sq = hsize*hsize
gen eligible = .
label var eligible "Eligible under PMT"
label define eligible 0 "Ineligible under PMT" 1 "Eligible under PMT"
// label define eligible 0 "Ineligible" 1 "Eligible"
...
gen poorest40 = .
replace poorest40 = 1 if consq == 1 | consq == 2
replace poorest40 = 0 if consq ~= 1 & consq ~= 2 & consq ~= .
label var poorest40 "Person is among the poorest 40% of the population"
label define poorest40 0 "No" 1 "Yes"
label values poorest40 poorest40
...
label var exemptfromcopayment "Exempt under current policy"
...
gen MBPFandexempt = .
replace MBPFandexempt = 1 if receives_MBPF == 1 & exemptfromcopayment == 1
replace MBPFandexempt = 0 if receives_MBPF == 0 | exemptfromcopayment == 0

// Who gets MBPF but not SGBP exempt?
gen MPBFbutnotexempt = .
replace MPBFbutnotexempt = 1 if receives_MBPF == 1 & exemptfromcopayment == 0
replace MPBFbutnotexempt = 0 if receives_MBPF == 0 | exemptfromcopayment == 1

// Who gets SGBP exempt but not MBPF?
gen noMBPFbutisexempt = .
replace noMBPFbutisexempt = 1 if receives_MBPF == 0 & exemptfromcopayment == 1
replace noMBPFbutisexempt = 0 if receives_MBPF == 1 | exemptfromcopayment == 0

// Who is in neither group?
gen noMBPFandnotexempt = .
replace noMBPFandnotexempt = 1 if receives_MBPF == 0 & exemptfromcopayment == 0
replace noMBPFandnotexempt = 0 if receives_MBPF == 1 | exemptfromcopayment == 1

forv x = 60/70 {
	gen eld`x' = .
	replace eld`x' = 1 if age >= `x' & age ~= .
	replace eld`x' = 0 if age <  `x' 
	label var eld`x' "Person is `x' years old or older"
	bys hhid: egen numeld`x' = sum(eld`x') 
	label var numeld`x' "Number of people in the house that are `x' years old or older"	
}

forv x = 5/18 {
	gen child`x' = .
	replace child`x' = 1 if age <  `x' 
	replace child`x' = 0 if age >= `x' & age ~= .
	label var child`x' "Person is under `x' years of age"
	bys hhid: egen numchild`x' = sum(child`x') 
	label var numchild`x' "Number of people in the house that are under `x' years of age"	
	gen numchild`x'_sq = numchild`x'*numchild`x'
	label var numchild`x'_sq "Square of number of people in the house that are under `x' years of age"
}

gen iswomanofreproductiveage = .
replace iswomanofreproductiveage = 1 if male == 0 & age >= 18 & age <= 45
replace iswomanofreproductiveage = 0 if male == 1 | age <  18 | age >  45
...
replace washingmachine = 1 if (asset18 > 0 & asset 18 ~= .) | (asset19 > 0 & asset19 ~= .)
gen refrigerator = 0
label var refrigerator "Fridge (1 chamber, or 2-3 chambers)"
replace refrigerator = 1 if (asset31 > 0 & asset 31 ~= .) | (asset32 > 0 & asset32 ~= .)
gen cartruckminivanscooter = 0
replace cartruckminivanscooter = 1 if ( (asset25 > 0 & asset25 ~= .) | (asset26 > 0 & asset26 ~= .) | (asset27 > 0 & asset27 ~= .) | (asset28 > 0 & asset28 ~= .) | (asset29 > 0 & asset29 ~= .) )
label var cartruckminivanscooter "Household has a car, truck, minivan or scooter"
...
foreach var of varlist computer mobilephone satelliteantenna asset* {
	tab `var', mi
	replace `var' = 1 if `var' >0 & `var' ~= .
	replace `var' = 0 if `var' ==.
	tab `var', mi	
}
label var asset17 "Drying machine, spin dryer" 
renamevarandvaluelabel asset17 spindryer 
...
preserve
use "$KG_PER_dir_kihs_2010_data2/f7_032_eng.dta", clear
rename hh_code hhid
collapseandpreserve (sum) land_size_m2=c3, by(hhid)
isid hhid
sort hhid
save "$WHO_KG_processed/land.dta",replace
restore
...
label var ovenfireplace "Household has oven or fireplace for cooking"
tab ovenfireplace, mi

renamevarandvaluelabel q1 typeofdwelling
renamevarandvaluelabel q4 howdidgetdwelling
renamevarandvaluelabel q6 livingspace
renamevarandvaluelabel q7 numberofrooms
renamevarandvaluelabel q9 yearofhouse
renamevarandvaluelabel q10 thoroughlyrepaird
renamevarandvaluelabel q11 yearthoroughlyrepaird
renamevarandvaluelabel q14 wallmaterial
renamevarandvaluelabel q15 roofmaterial
renamevarandvaluelabel q16r1c2 centralheating
renamevarandvaluelabel q16r1c3 centralheating_operational
renamevarandvaluelabel q16r2c2 indivheating
renamevarandvaluelabel q16r2c3 indivheating_operational
renamevarandvaluelabel q16r3c2 watersupply
renamevarandvaluelabel q16r3c3 watersupply_operational
renamevarandvaluelabel q16r4c2 sewage
renamevarandvaluelabel q16r4c3 sewage_operational
renamevarandvaluelabel q16r5c2 hotwatersupply
renamevarandvaluelabel q16r5c3 hotwatersupply_operational
renamevarandvaluelabel q16r6c2 centralgassupply
renamevarandvaluelabel q16r6c3 centralgassupply_operational
renamevarandvaluelabel q16r7c2 bathorshower
renamevarandvaluelabel q16r7c3 bathorshower_operational
renamevarandvaluelabel q16r8c2 telephone
renamevarandvaluelabel q16r8c3 telephone_operational
renamevarandvaluelabel q16r9c2 electriccooking
renamevarandvaluelabel q16r9c3 electriccooking_operational
renamevarandvaluelabel q16r10c2 powersupply
renamevarandvaluelabel q16r10c3 powersupply_operational
renamevarandvaluelabel q16r11c2 elevator
renamevarandvaluelabel q16r11c3 elevator_operational
renamevarandvaluelabel q16r12c2 garbagechute
renamevarandvaluelabel q16r12c3 garbagechute_operational
renamevarandvaluelabel q17 gasmeterinstalled
renamevarandvaluelabel q19 coldwatermeterinstalled
renamevarandvaluelabel q20 coldwatermetershared
renamevarandvaluelabel q22 electricitymeter
renamevarandvaluelabel q23 electricitymetershared
renamevarandvaluelabel q25 mainwatersource
renamevarandvaluelabel q26 locationwatersource
renamevarandvaluelabel q31 typeofbathroom
renamevarandvaluelabel q33 bathroomshared

// the type of additional house seems mutually exclusive enough = there is only one observation that overlaps for a3 and a4
gen additionaldwellingandtype = .
replace additionaldwellingandtype = 0 if q40 == 2
replace additionaldwellingandtype = 1 if q41_a1 == 1
replace additionaldwellingandtype = 2 if q41_a2 == 2
replace additionaldwellingandtype = 3 if q41_a3 == 3
replace additionaldwellingandtype = 4 if q41_a4 == 4
replace additionaldwellingandtype = 5 if q41_a5 == 5
label define additionaldwellingandt 0 "No additional dwelling" 1 "Separate apartment" 2 "House or half-house in urban area" 3 "House or half-house in rural area" 4 "Summer cottage" 5 "Other" 
label values additionaldwellingandt additionaldwellingandtype
...
megaif 1 2 4, c(replace wallsnotbrickconcretewood = 0) v(wallmaterial)
megaif 1 2 4, c(replace wallsnotbrickconcretewood = 1) v(wallmaterial) e("~=") s("&")
tab wallsnotbrickconcretewood

tab q32, mi
gen bathroomoutsidehouse = .
label var bathroomoutsidehouse "Bathroom is outside the house"
...
tab q40
gen additionalhouse = .
label var additionalhouse "Household has additional house"
replace additionalhouse = 1 if q40 == 1
replace additionalhouse = 0 if q40 == 2
tab additionalhouse

gen hasgasheating = .
replace hasgasheating = 1 if q28_a5 == 5
replace hasgasheating = 0 if q28_a5 == .
label var hasgasheating "Household has gas heating"
...
// Merge in land
sort hhid
merge m:1 hhid using "$WHO_KG_processed/land.dta"
tab _m
keep if _m ~= 2
drop _m

gen ln_landsize = ln(land_size_m2)
gen livingroomsperperson = numberofrooms/hsize
...
// save the dataset to use for analysis
sort hhid idcode
svyset hhid [pw=expfact], strata(strata) 
save "$WHO_KG_processed/health2010_kihs_relabeled_merged_12-04-2012.dta", replace

// testing...
svy: reg logpccd hsize

// save a household-level dataset
keep if reltohead == 1
svyset hhid [pw=hhw], strata(strata) 
save "$WHO_KG_processed/health2010_kihs_relabeled_merged_hhlevel_12-04-2012.dta", replace
...
//	page 48 "In the process of implementing the SGBP, there has been a considerable increase in the population categories entitled to various forms of 
ion, from 29 categories in 2001 to 72 in 2009. This increase in the number of exempt categories was accompanied by an increase in the number of patients entitled to exemptions...
