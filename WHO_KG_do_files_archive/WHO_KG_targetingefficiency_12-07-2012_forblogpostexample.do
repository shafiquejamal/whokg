...
append using "$WHO_KG_reports/dataset_coverage_all_21850.dta"
gen ordertoplot = 0
replace ordertoplot = 2 if category == "Current Policy (covers 46% of population)"
replace ordertoplot = 3 if category == "MBPF (covers 10% of population)"
replace ordertoplot = 1 if category == "PMT (covers 46% of population)"
...
graph bar (asis) coverage, over(category, sort(ordertoplot)) over(quantile) asyvars bar(1, color(green)) bar(2, color(blue)) bar(3, color(orange)) b1title("Quintile") title("Coverage Rates of Copayment Exemptions") subtitle("Current Policy, MBPF and a preliminary PMT") note("Source: KIHS 2010 data and WHO Calculations, `datetouse'") blabel(total, format(%9.0f)) ytitle("Coverage (%)", margin(medium)) legend(size(small) `relabellegend') 
...
*/
label var exempt "Current Policy"
rename exempt currentpolicy
label var receives "Alternative A"
rename receives_ alternative_a
label var eligible "Alternative B"
rename eligible alternative_b
...
graph export "$WHO_KG_reports/Graph_mean_11-14-2012_example_for_blogpost1.png", replace
...
graph export "$WHO_KG_reports/Graph_mean_11-14-2012_example_for_blogpost2.png", replace
...
graph export "$WHO_KG_reports/Graph_mean_11-14-2012_example_for_blogpost3.png", replace
overlappingcatgraphmean pccd using "$WHO_KG_reports/eraseme.dta", gc(graph bar (asis)) catvarlist("currentpolicy alternative_a alternative_b") 	v(0 "Non-beneficiary" 1 "Beneficiary") over2options(lab(angle(0) labs(vsmall))) replace /* 
*/ go(note(`"Source: Some data source "') asy title("Average consumption of groups within population") subtitle("Simulation of policy options") ytitle("Per capita HH consumption (LCU)", margin(medium)) legend(size(small)) blabel(total, format(%9.0fc))) 
graph export "$WHO_KG_reports/Graph_mean_11-14-2012_example_for_blogpost.png", replace

// distribution of exempt and non-exempt
// % of exempt, non-exempt in each quintile
tab consq exemptfrom [aw=expfact], col mi
taboutgraph consq exemptfrom [aw=expfact]    using "$WHO_KG_reports/composition_distribution_exemptfromcopayments.csv", gc(`"graph bar"') ta(cells(col) f(2 2 2 2)) replace go( note("Source: KIHS 2010 data and WHO Calculations, `datetouse'") b1title("Quintile") title(`"Distribution of Population"') subtitle("Current Exemption Policy") ytitle("Percent of population in the quntile", margin(medium)) legend(size(small)) blabel(total, format(%9.0fc)))
...
taboutgraph consq receives_MBPF [aw=expfact] using "$WHO_KG_reports/composition_distribution_receives_MBPF.csv",                                 gc(`"graph bar"') ta(cells(col) f(2 2 2 2)) replace go( note("Source: KIHS 2010 data and WHO Calculations, `datetouse'") b1title("Quintile") title(`"Distribution of Population"') subtitle("MBPF") ytitle("Percent of population in the quntile", margin(medium)) legend(size(small)) blabel(total, format(%9.0fc))) 

// distribution of PMT eligible and ineligible
// % of PMT==1, PMT==0 in each quintile
tab consq eligible [aw=expfact], col mi
// tabout consq receives_MBPF [aw=expfact] using "$WHO_KG_reports/receives_MBPF.csv", cells(col) f(2 2 2 2) replace
taboutgraph consq eligible [aw=expfact] using "$WHO_KG_reports/composition_distribution_PMTeligible.csv",                                 gc(`"graph bar"') ta(cells(col) f(2 2 2 2)) replace go( note("Source: KIHS 2010 data and WHO Calculations, `datetouse'") b1title("Quintile") title(`"Distribution of Population"') subtitle("Using PMT") ytitle("Percent of population in the quntile", margin(medium)) legend(size(small)) blabel(total, format(%9.0fc))) 
