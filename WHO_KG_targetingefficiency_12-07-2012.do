...
gen ordertoplot = 0
replace ordertoplot = 2 if category == "Current Policy (covers 47.6% of population)"
...
replace coverage = coverage * 100
encode category, gen(catn)
graph bar (asis) coverage, over(category) over(quantile) asyvars bar(1, color(green)) b1title("Quintile") title("Coverage Rates of Copayment Exemptions") subtitle("Current Policy") note("Source: KIHS 2010 data and WHO Calculations, `datetouse'") blabel(total, format(%9.0f)) ytitle("Coverage (%)", margin(medium)) legend(size(small)) 
